...
description: 在 plan package 的 `spec.md`、`research.md`、`truth-delta.md` 與 `specs/truth/**` 基礎上，盤點本次需求涉及的系統介面與分析 wave，產出 plan-side `plan.md`，並把 plan package、truth root、truth-delta path 傳給 `/axb-api-plan`、`/data-plan`、`/ui-plan`。
...
2. DELEGATE 依 Wave 順序將 API 介面交給 `/axb-api-plan`、資料介面交給 `/data-plan`、UI 介面交給 `/ui-plan`；每次 handoff 都必須包含 plan package path、truth root、truth-delta path、介面名稱與分析重點。
